  ...
  if (restoreId) {
    ...
    if (existing && isChecklist(existing)) {
      ...
      membros.value = [
        ...
        ...Array.from({ length: Math.max(0, 4 - existingMembros.length) }, () => ({ nome: "", matricula: "" })),
      ];
...
function abrirModalNaoConforme(pergunta: PerguntaGoman) {
  ...
  // Edição de NC existente com foto+obs já preenchidos → vai direto para passo 2
