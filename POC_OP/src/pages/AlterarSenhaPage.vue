    ...
    <main class="rp-main">
      <div class="rp-card">
        ...
        <template v-if="done">
          ...
          <p class="rp-title">Senha alterada!</p>
          <p class="rp-sub">Sua senha foi atualizada com sucesso. Você já pode usar o sistema normalmente.</p>
          <button class="rp-btn" @click="router.replace('/')">Entrar no sistema</button>
        </template>

        <template v-else>
          <p class="rp-title">Defina sua senha</p>
          <p class="rp-sub">Este é seu primeiro acesso. Crie uma senha pessoal para continuar.</p>

          <div class="rp-field">
            <label class="rp-lbl" for="pw1">NOVA SENHA</label>
            <div class="rp-input-wrap">
              ...
              />
              <button type="button" class="rp-eye" @click="show1 = !show1">
          ...
          <div class="rp-field">
            <label class="rp-lbl" for="pw2">CONFIRMAR SENHA</label>
            <div class="rp-input-wrap">
              <input
                id="pw2"
                ref="pw2Ref"
                ...
                @keyup.enter="submit"
              />
              <button type="button" class="rp-eye" @click="show2 = !show2">
          ...
          <button class="rp-btn" :disabled="!canSubmit || loading" @click="submit">
            <template v-if="!loading">Salvar e entrar</template>
...
import { clearMustChangePassword, clearRoleCache } from "@/lib/role";
...
const canSubmit = computed(() => pw1.value.length >= 8 && pw1.value === pw2.value);

onMounted(() => nextTick(() => pw1Ref.value?.focus()));
...
async function submit() {
  ...
  if (data.session) {
    await clearMustChangePassword(data.session.user.id);
    clearRoleCache();
  }
...
.rp-card { width: 100%; max-width: 340px; display: flex; flex-direction: column; }
.rp-icon {
...
.rp-strength__bar { flex: 1; height: 2px; border-radius: 2px; transition: width .3s, background .3s; }
